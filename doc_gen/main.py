...
def main_util(item_to_scan, doc_type, skip_pattern, recursive, file_summary_info):
    glob_to_skip = (
        glob.glob(skip_pattern, recursive=recursive)
        if type(skip_pattern) != list
        else skip_pattern
    )
    ...
    if doc_type == TAG_VALUE:
        spdx_file = SPDXFile(
            project_path, spdx_file_name, allIdentifiers, file_summary_info, TAG_VALUE
        )
        spdx_file.create()
    else:
        spdx_file = SPDXFile(
            project_path, spdx_file_name, allIdentifiers, file_summary_info, RDF
        )
        spdx_file.create()
